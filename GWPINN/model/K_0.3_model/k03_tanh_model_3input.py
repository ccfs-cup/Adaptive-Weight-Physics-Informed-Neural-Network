...
from dataset.k03.getdatak03 import lc_k03_points2, k03_hmax, get_k03data, pdexytpoints, extract_ic_data,Npde_k03
...
checkpoint_save_path = "/home/cc/CCFs/Wangf/GWPGNN/checkpoints/k03/k03_weights.ckpt"
if not os.path.exists(checkpoint_save_path):
    os.makedirs(checkpoint_save_path)

log_dir = './logs/k03/' + current_time  # 日志文件地址
summary_writer = tf.summary.create_file_writer(log_dir)
tf.summary.trace_on(graph=True, profiler=True)

save_dir = '/home/cc/CCFs/Wangf/GWPGNN/savepick/savepick03'  # 存放验证图片的路径
...
def LossPde(re_pde_xyt, pde_weights):
    ...
    loss_pde = tf.reduce_mean(tf.square(
        pde_weights*(0.1*dh_dt - 0.3*(dh_dx*dh_dx + pde_h_out*d2h_dx2) - 0.3*(dh_dy*dh_dy + pde_h_out*d2h_dy2))))  # k是常数 dk/dx=dk/dy=0
...
def LossLogits(train_xyt,train_h,ic_xyt,ic_h,ic_weights):
    ...
    loss_ek = tf.sqrt(tf.reduce_mean(tf.square(tf.abs(tf.nn.relu(logits_train - k03_hmax/1.5))))+ 1e-7) 
...
#数据
#已归一化
(train_xyt, train_h), (validation_xyt, validation_h), (test_xyt, test_h) = get_k03data()
...
#权重设置
pde_weights = tf.Variable(tf.reshape(tf.repeat(10000.0, Npde_k03),(Npde_k03, -1)))
ic_weights = tf.Variable( 100 * tf.random.uniform([526, 1]))
...
def trainmain():
    ...
    for epoch in range(epochs):
        ...
        h_label = test_h
        #差异热图
        plot_compared_heatmaps_3pic1(lc_k03_points2, h_label*1.5 , h_model*1.5, epoch, save_dir, current_time)
