...
from dataset.hru1.makedata_k_23 import get_data,get_data1
...
checkpoint_save_path = "/home/cc/CCFs/Wangf/GWPGNN/checkpoints/hru1/hru1_sin_4input/hru1_weights_sin.ckpt"
...
save_dir = './pics/hru1_k_23/val_pics_k_23/7_4_70cell_4input'  # 存放验证图片的路径
...
def LossLogits(train_xyt, ic_xyt, bd_xyt, train_h, ic_heads, h_bd):
    ...
    loss_ek = tf.sqrt(tf.reduce_mean(tf.square(tf.abs(tf.nn.relu(logits_bd - 1))))+ 1e-7) 
    # 初始条件损失1
    # loss_ic = tf.reduce_mean(tf.square(logits_ic - ic_heads))
    loss_ic = tf.sqrt(tf.reduce_mean(tf.square(logits_ic - ic_heads))+ 1e-7) 
    # 边界条件损失   给定水头边界  没有给定水头边界      哪来的给定水头边界啊  我真服了  都做的什么实验啊你
...
#训练数据
#已归一化
(train_xyt, train_h), (ic_xyt, ic_heads), (bd_xyt, h_bd), (test_xyt, test_h),(hmax_value,hmin_value) = get_data1()
...
# Define the custom layer with the sin activation function
...
class ResidualBlock(keras.layers.Layer):
    def __init__(self, inp_dim, outp_dim):
        ...
        self.fc1 = MyDense(inp_dim, outp_dim)
        self.fc2 = MyDense(outp_dim, outp_dim)

    def call(self, inputs, training=None):
        x = self.fc1(inputs)
        x = tf.math.sin(x)  # 使用sin作为激活函数
        x = self.fc2(x)
        out = x + inputs  # 添加残差连接
...
class MyModel(keras.Model):
    def __init__(self):
        ...
        self.fc1 = MyDense(4, 70)
        self.fc2 = ResidualBlock(70, 70)
        self.fc3 = ResidualBlock(70, 70)
        self.fc4 = MyDense(70, 1)
