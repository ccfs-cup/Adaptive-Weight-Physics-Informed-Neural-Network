...
def plot_loss(epochs, total_losses,train_losses, val_losses):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'{save_dir_loss}/loss_sigmoid_80cell{current_time}.png')
...
def LossLogits(train_xyt, ic_xyt, bd_xyt, train_h, ic_heads, h_bd):
    ...
    loss_ek = tf.sqrt(tf.reduce_mean(tf.square(tf.abs(tf.nn.relu(logits_bd - 1))))+ 1e-20) 
    # 初始条件损失
    # loss_ic = tf.reduce_mean(tf.square(logits_ic - ic_heads))
    loss_ic = tf.sqrt(tf.reduce_mean(tf.square(logits_ic - ic_heads))+ 1e-20) 
    # 边界条件损失   给定水头边界
    # loss_bc = tf.reduce_mean(tf.square(logits_bd - h_bd))
    loss_bc = tf.sqrt(tf.reduce_mean(tf.square(logits_bd - h_bd))+ 1e-20) 
    # 训练数据损失
    # loss_mse = tf.reduce_mean(tf.losses.MSE(logits_train, train_h))
    loss_mse =tf.sqrt(tf.reduce_mean(tf.losses.MSE(logits_train, train_h))+ 1e-20) 
...
class MyModel(keras.Model):
    def __init__(self):
        ...
        #80cell
        self.fc1 = MyDense(3, 80)
        self.fc2 = MyDense(80, 80)
        self.fc3 = MyDense(80, 80)
        self.fc4 = MyDense(80, 80)
        self.fc5 = MyDense(80, 80)
        self.fc6 = MyDense(80, 80)
        self.fc7 = MyDense(80, 1)
...
def trainmain():
    ...
    for epoch in range(epochs):
        ...
        loss_val = tf.sqrt(tf.reduce_mean(tf.square(logits_val - test_h))+ 1e-10)
