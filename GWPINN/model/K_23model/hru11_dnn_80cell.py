...
save_dir = './pics/hru1_k_23/val_pics_k_23/2024_6_20_80cell'  # 存放验证图片的路径
save_dir1 = './pics/hru1_k_23/val_pics_k_23/6_20_80cell_valstep1'
save_dir2 = './pics/hru1_k_23/val_pics_k_23/6_20_80cell_valstep2'
...
def plot_loss(epochs, total_losses,train_losses, val_losses):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'{save_dir_loss}/loss_sigmoid_60cell{current_time}.png')
...
def plot_compared_heatmaps_3pic2(lc_hru1, val_h, modeloutput, epoch, save_dir1, current_time):
    # 确保 val_h 和 modeloutput 的长度与 lc_hru1 一致
    val_h = val_h[:lc_hru1.shape[0], :]
    modeloutput = modeloutput[:lc_hru1.shape[0], :]
    ...
    # 保存图片到指定路径，文件名以 epoch 值命名
    plt.savefig(f'{save_dir1}/heatmap_epoch_{epoch}_{current_time}.png')
    plt.close()  # 关闭图像，避免重叠显示
    
def plot_compared_heatmaps_3pic3(lc_hru1, val_h, modeloutput, epoch, save_dir2, current_time):
    # 确保 val_h 和 modeloutput 的长度与 lc_hru1 一致
    val_h = val_h[lc_hru1.shape[0]:2*lc_hru1.shape[0], :]
    modeloutput = modeloutput[lc_hru1.shape[0]:2*lc_hru1.shape[0], :]
    ...
    # 保存图片到指定路径，文件名以 epoch 值命名
    plt.savefig(f'{save_dir2}/heatmap_epoch_{epoch}_{current_time}.png')
    plt.close()  # 关闭图像，避免重叠显示
...
class MyModel(keras.Model):
    def __init__(self):
        ...
        #60cell
        self.fc1 = MyDense(3, 60)
        self.fc2 = MyDense(60, 60)
        self.fc3 = MyDense(60, 60)
        self.fc4 = MyDense(60, 60)
        self.fc5 = MyDense(60, 60)
        self.fc6 = MyDense(60, 60)
        self.fc7 = MyDense(60, 1)
...
def trainmain():
    ...
    for epoch in range(epochs):
        ...
        train_losses.append(avg_epoch_train_loss)  # 记录每个epoch平均train_loss损失
        ...
        plot_compared_heatmaps_3pic2(lc_hru1_points, h_label , h_model, epoch, save_dir1, current_time)
        plot_compared_heatmaps_3pic3(lc_hru1_points, h_label , h_model, epoch, save_dir2, current_time)
