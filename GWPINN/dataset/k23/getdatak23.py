...
#1、提取k23区域内所有网格坐标及对应276时间步水头值
k23_fn = './coordinate data/HRU1区域坐标文件/HRU1区域内全部点坐标.xlsx'
suffix = '_minus_1'  # 减一
k23_r_c = pre_data(k23_fn, suffix, skiprows=2, header=None, usecols=None)
# lc_k23是k23区域内所有网格的坐标集合(minus_1)  lc_k23.shape:(809,2)
lc_k23 = get_points(k23_r_c)
lc_k23_points = lc_k23
lc_k23_points2 = lc_k23+1  #画图用，按照原刻度
...
#得到最大值
max_xyt, max_h = extract_maxdata(lc_k23_points)
k23_hmax = np.max(max_h)  #1.5495648
# print(k23_hmax)

# 数据提取函数   k_23: data points number = 320
def extract_train_data(time_steps,points):
    indices = np.linspace(0, len(points)-1, 320, dtype=int)
    train_points = lc_k23_points[indices]
    xyt = []
    h = []
    for t in time_steps:
...
train_xyt, train_h = extract_train_data(train_time_steps,lc_k23_points)
validation_xyt, validation_h = extract_validation_data(validation_time_steps,lc_k23_points)
test_xyt, test_h = extract_test_data(test_time_steps,lc_k23_points)
train_xyt = train_xyt + 1
validation_xyt = validation_xyt + 1
test_xyt = test_xyt + 1

# 数据保存
np.savez('/home/cc/CCFs/Wangf/GWPGNN/dataset/k23/k23_npz/k23_reshape.npz', train_xyt=train_xyt,
                 train_h=train_h, validation_xyt=validation_xyt, validation_h=validation_h, test_xyt=test_xyt, test_h=test_h)



def get_k23data():
    data1 = np.load('/home/cc/CCFs/Wangf/GWPGNN/dataset/k23/k23_npz/k23_reshape.npz')
...
# (train_xyt1, train_h1), (validation_xyt1, validation_h1), (test_xyt1, test_h1)=get_K23data()
...
def pdexytpoints(): 
    # np.random.seed(42) 
    # N_pde = 800
    # pde_rad_id = np.random.choice(lc_k23.shape[0], size=N_pde, replace=False)  # 生成随机索引
    # sel_poi = lc_k23[pde_rad_id, :]  # 生成随机pde点
    # k23_pde = sel_poi 
    pde_xyt = []
    for t in range(4,276):
        for (i,j) in lc_k23_points:
...
Npde_k23 = test_pde.shape[0]

 
def extract_daman():
    xyt = []
    h = []
    for t in range(228,276):
        xyt.append([93, 112, t-4])
        h.append([max(heads_data[93, 112, t], 0)])
