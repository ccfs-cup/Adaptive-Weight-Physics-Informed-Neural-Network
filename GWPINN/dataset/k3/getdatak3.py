...
#1、提取k3区域内所有网格坐标及对应276时间步水头值
k3_fn = './coordinate data/K_3区域坐标文件/K_3区域全部点坐标.xlsx'
suffix = '_minus_1'  # 减一
k3_r_c = pre_data(k3_fn, suffix, skiprows=2, header=None, usecols=None)
# lc_k3是k3区域内所有网格的坐标集合(minus_1)  lc_k3.shape:(329,2)
lc_k3 = get_points(k3_r_c)
lc_k3_points = lc_k3
# print(lc_k3.shape)
lc_k3_points2 = lc_k3+1  #画图用，按照原刻度
...
#得到最大值
max_xyt, max_h = extract_maxdata(lc_k3_points)
k3_hmax = np.max(max_h)  #1.4326819
# print(k3_hmax)

# 数据提取函数   k_23: data points number = 130
def extract_train_data(time_steps,points):
    indices = np.linspace(0, len(points)-1, 130, dtype=int)
    train_points = lc_k3_points[indices]
    xyt = []
    h = []
    for t in time_steps:
...
train_xyt, train_h = extract_train_data(train_time_steps,lc_k3_points)
validation_xyt, validation_h = extract_validation_data(validation_time_steps,lc_k3_points)
test_xyt, test_h = extract_test_data(test_time_steps,lc_k3_points)
train_xyt = train_xyt + 1
validation_xyt = validation_xyt + 1
test_xyt = test_xyt + 1

# 数据保存
np.savez('/home/cc/CCFs/Wangf/GWPGNN/dataset/k3/k3_npz/k3_reshape.npz', train_xyt=train_xyt,
                 train_h=train_h, validation_xyt=validation_xyt, validation_h=validation_h, test_xyt=test_xyt, test_h=test_h)



def get_k3data():
    data1 = np.load('/home/cc/CCFs/Wangf/GWPGNN/dataset/k3/k3_npz/k3_reshape.npz')
...
(train_xyt1, train_h1), (validation_xyt1, validation_h1), (test_xyt1, test_h1)=get_k3data()
...
def pdexytpoints(): 
    # np.random.seed(42) 
    # N_pde = 200
    # pde_rad_id = np.random.choice(lc_k3.shape[0], size=N_pde, replace=False)  # 生成随机索引
    # sel_poi = lc_k3[pde_rad_id, :]  # 生成随机pde点
    # k3_pde = sel_poi 
    pde_xyt = []
    for t in range(4,276):
        for (i,j) in lc_k3_points:
