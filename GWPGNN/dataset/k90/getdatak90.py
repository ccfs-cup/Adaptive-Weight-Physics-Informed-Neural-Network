...
#1、提取k90区域内所有网格坐标及对应276时间步水头值
k90_fn = './coordinate data/k_90区域坐标文件/K_90区域全部点坐标.xlsx'
suffix = '_minus_1'  # 减一
k90_r_c = pre_data(k90_fn, suffix, skiprows=2, header=None, usecols=None)
# lc_k90是k90区域内所有网格的坐标集合(minus_1)  lc_k90.shape:(702,2)
lc_k90 = get_points(k90_r_c)
lc_k90_points = lc_k90
# print(lc_k90.shape)
lc_k90_points2 = lc_k90+1  #画图用，按照原刻度
...
#得到最大值
max_xyt, max_h = extract_maxdata(lc_k90_points)
k90_hmax = np.max(max_h)  #1.4865059
# print(k90_hmax)

# 数据提取函数   k_23: data points number = 270
def extract_train_data(time_steps,points):
    indices = np.linspace(0, len(points)-1, 270, dtype=int)
    train_points = lc_k90_points[indices]
    xyt = []
    h = []
    for t in time_steps:
...
train_xyt, train_h = extract_train_data(train_time_steps,lc_k90_points)
validation_xyt, validation_h = extract_validation_data(validation_time_steps,lc_k90_points)
test_xyt, test_h = extract_test_data(test_time_steps,lc_k90_points)
train_xyt = train_xyt + 1
validation_xyt = validation_xyt + 1
test_xyt = test_xyt + 1

# 数据保存
np.savez('/home/cc/CCFs/Wangf/GWPGNN/dataset/k90/k90_npz/k90_reshape.npz', train_xyt=train_xyt,
                 train_h=train_h, validation_xyt=validation_xyt, validation_h=validation_h, test_xyt=test_xyt, test_h=test_h)



def get_k90data():
    data1 = np.load('/home/cc/CCFs/Wangf/GWPGNN/dataset/k90/k90_npz/k90_reshape.npz')
...
# (train_xyt1, train_h1), (validation_xyt1, validation_h1), (test_xyt1, test_h1)=get_k90data()
...
def pdexytpoints(): 
    # np.random.seed(42) 
    # N_pde = 400
    # pde_rad_id = np.random.choice(lc_k90.shape[0], size=N_pde, replace=False)  # 生成随机索引
    # sel_poi = lc_k90[pde_rad_id, :]  # 生成随机pde点
    # k90_pde = sel_poi 
    pde_xyt = []
    for t in range(4,276):
        for (i,j) in lc_k90_points:
