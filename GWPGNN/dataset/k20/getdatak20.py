...
# heads_max = np.max(heads_data[:,:,3:272])
# print(heads_max)  #1549.5648193359375

#提取xyt和h,并保存为npz文件
      
#1、提取k20区域内所有网格坐标及对应276时间步水头值
k20_fn = './coordinate data/K_20区域坐标文件/K_20区域全部点坐标.xlsx'
suffix = '_minus_1'  # 减一
k20_r_c = pre_data(k20_fn, suffix, skiprows=2, header=None, usecols=None)
# lc_k20是k20区域内所有网格的坐标集合(minus_1)  lc_k20.shape:(3614,2)
lc_k20 = get_points(k20_r_c)
lc_k20_points = lc_k20
# print(lc_k20.shape)
lc_k20_points2 = lc_k20+1  #画图用，按照原刻度
...
#得到最大值
max_xyt, max_h = extract_maxdata(lc_k20_points)
k20_hmax = np.max(max_h)  #1.4446337
# print(k20_hmax)

# 数据提取函数   k_23: data points number = 1400
def extract_train_data(time_steps,points):
    indices = np.linspace(0, len(points)-1, 1400, dtype=int)
    train_points = lc_k20_points[indices]
    xyt = []
    h = []
    for t in time_steps:
...
train_xyt, train_h = extract_train_data(train_time_steps,lc_k20_points)
validation_xyt, validation_h = extract_validation_data(validation_time_steps,lc_k20_points)
test_xyt, test_h = extract_test_data(test_time_steps,lc_k20_points)
train_xyt = train_xyt + 1
validation_xyt = validation_xyt + 1
test_xyt = test_xyt + 1

# 数据保存
np.savez('/home/cc/CCFs/Wangf/GWPGNN/dataset/k20/k20_npz/k20_reshape.npz', train_xyt=train_xyt,
                 train_h=train_h, validation_xyt=validation_xyt, validation_h=validation_h, test_xyt=test_xyt, test_h=test_h)



def get_k20data():
    data1 = np.load('/home/cc/CCFs/Wangf/GWPGNN/dataset/k20/k20_npz/k20_reshape.npz')
...
(train_xyt1, train_h1), (validation_xyt1, validation_h1), (test_xyt1, test_h1)=get_k20data()
...
def pdexytpoints(): 
    # np.random.seed(42) 
    # N_pde = 1500
    # pde_rad_id = np.random.choice(lc_k20.shape[0], size=N_pde, replace=False)  # 生成随机索引
    # sel_poi = lc_k20[pde_rad_id, :]  # 生成随机pde点
    # k20_pde = sel_poi 
    pde_xyt = []
    for t in range(4,276):
        for (i,j) in lc_k20_points:
