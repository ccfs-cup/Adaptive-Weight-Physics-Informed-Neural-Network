...
from dataset.k90.getdatak90 import lc_k90_points2, k90_hmax, get_k90data, pdexytpoints, extract_ic_data,Npde_k90
...
checkpoint_save_path = "/home/cc/CCFs/Wangf/GWPGNN/checkpoints/k90/k90_weights.ckpt"
if not os.path.exists(checkpoint_save_path):
    os.makedirs(checkpoint_save_path)

log_dir = './logs/k90/' + current_time  # 日志文件地址
summary_writer = tf.summary.create_file_writer(log_dir)
tf.summary.trace_on(graph=True, profiler=True)

save_dir = '/home/cc/CCFs/Wangf/GWPGNN/savepick/savepick90'  # 存放验证图片的路径
...
def LossPde(re_pde_xyt, pde_weights):
    ...
    loss_pde = tf.reduce_mean(tf.square(
        pde_weights*(0.1*dh_dt - 90*(dh_dx*dh_dx + pde_h_out*d2h_dx2) - 90*(dh_dy*dh_dy + pde_h_out*d2h_dy2))))  # k是常数 dk/dx=dk/dy=0
...
def LossLogits(train_xyt,train_h,ic_xyt,ic_h,ic_weights):
    ...
    loss_ek = tf.sqrt(tf.reduce_mean(tf.square(tf.abs(tf.nn.relu(logits_train - k90_hmax/1.5))))+ 1e-7) 
...
#数据
#已归一化
(train_xyt, train_h), (validation_xyt, validation_h), (test_xyt, test_h) = get_k90data()
...
#权重设置
pde_weights = tf.Variable(tf.reshape(tf.repeat(10000.0, Npde_k90),(Npde_k90, -1)))
ic_weights = tf.Variable( 10 * tf.random.uniform([702, 1]))
...
def trainmain():
    ...
    for epoch in range(epochs):
        ...
        h_label = test_h
        #差异热图
        plot_compared_heatmaps_3pic1(lc_k90_points2, h_label*1.5 , h_model*1.5, epoch, save_dir, current_time)
