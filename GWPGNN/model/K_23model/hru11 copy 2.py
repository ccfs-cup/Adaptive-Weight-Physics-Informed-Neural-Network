...
save_dir = './pics/hru1_k_23/test'  # 存放验证图片的路径
...
def plot_loss(epochs, train_losses, val_losses):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'{save_dir_loss}/loss_{current_time}.png')
    plt.close()  # 关闭图像，避免重叠显示

     
def plot_compared_heatmaps_3pic(lc_hru1, val_h, modeloutput, epoch, save_dir, current_time):
    ...
    # 计算差值矩阵
    diff_data_matrix = abs(val_data_matrix - modeloutput_data_matrix)

    # 确定颜色映射范围
    vmin = min(np.nanmin(val_data_matrix), np.nanmin(modeloutput_data_matrix))
    vmax = max(np.nanmax(val_data_matrix), np.nanmax(modeloutput_data_matrix))
    ...
    plt.imshow(val_data_matrix, cmap='viridis',
               interpolation='nearest', origin='upper', vmin=vmin, vmax=vmax)
    plt.colorbar()
    ...
    plt.imshow(diff_data_matrix, cmap='coolwarm',
...
def LossLogits(train_xyt, ic_xyt, bd_xyt, train_h, ic_heads, h_bd):
    ...
    loss_ek = tf.reduce_mean(
        tf.square(tf.abs(tf.nn.relu(logits_bd - 1))))
    # 初始条件损失
...
plot_compared_heatmaps_3pic1(lc_hru1_points, test_h , logits_val, 11, save_dir, current_time)
...
#         # plot_compared_heatmaps(lc_hru1, h_label , h_model, epoch, save_dir, current_time)
#         # plot_compared_heatmaps1(lc_hru1, h_label , h_model, epoch, save_dir, current_time)
        
#         #差异热图
#         plot_compared_heatmaps_3pic(lc_hru1_points, h_label , h_model, epoch, save_dir, current_time)
