...
def loss(x_f_batch, t_f_batch,
    ...
    f_u_pred = f_model(x_f_batch, t_f_batch)   # return f_u ： f_u = f_u = u_t + u*u_x - (0.01/tf.constant(math.pi))*u_xx
    u0_pred = u_model(tf.concat([x0, t0],1))
    u_lb_pred, _ = u_x_model(x_lb, t_lb)   # return u,u_x
    ...
    mse_b_u = tf.reduce_mean(tf.square(u_lb_pred - 0)) + \
            tf.reduce_mean(tf.square(u_ub_pred - 0)) #since ub/lb is 0   上边界和下边界是0
...
def f_model(x,t):
    with tf.GradientTape(persistent=True) as tp0:
        ...
        tp0.watch([t])
        u = u_model(tf.concat([x,t],1))
        u_x = tp0.gradient(u,x)
        u_t = tp0.gradient(u,t)
    u_xx = tp0.gradient(u_x,x)
    f_u = u_t + u*u_x - (0.01/tf.constant(math.pi))*u_xx
    del tp0  #当已经完成所有的梯度计算后，通常最好删除持久的梯度带来节省内存。即，在所有梯度操作之后，添加del tp0
    return f_u
...
def fit(x_f, t_f, x0, t0, u0, x_lb, t_lb, x_ub, t_ub, col_weights, u_weights, epochs, newton_iter):
    # Built in support for mini-batch, set to N_f (i.e. full batch) by default
    N_f = 10000
    batch_sz = N_f    #默认使用full batch 全批量    可以改为小批量 
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            ...
            tf.print(f"mse_0: {mse_0},mse_f: {mse_f},total loss: {loss_value},每10个epoch时间:{elapsed}")
            start_time = time.time()
...
def get_loss_and_flat_grad(x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch, x_lb, t_lb, x_ub, t_ub, col_weights, u_weights):
    def loss_and_flat_grad(w):
        ...
        grad_flat = tf.concat(grad_flat, 0)   # tf.concat: 将多个张量沿指定轴拼接
...
# data = scipy.io.loadmat('/home/cc/CCFs/Wangf/SA-PINNs-master/SA-PINNs-master/Burgers/burgers_shock.mat')
data = scipy.io.loadmat('SA-PINNs-master/Burgers/burgers_shock.mat')


t = data['t'].flatten()[:,None]   #(100,1)
x = data['x'].flatten()[:,None]  #(256,1)
# Exact = data['usol']
# Exact_u = np.real(Exact)   # Exact = np.real(data['usol'])   np.real 返回数组的实部
Exact_u = data['usol']  #(256,100)


#grab random points off the initial condition
idx_x = np.random.choice(x.shape[0], N0, replace=False)   #随机选取x中100个不重复的索引
x0 = x[idx_x,:] 
u0 = tf.cast(Exact_u[idx_x,0:1], dtype = tf.float32)

idx_t = np.random.choice(t.shape[0], N_b, replace=False)
tb = t[idx_t,:]   #得到t中的25个随机的点作为tb

# Sample collocation points via LHS
X_f = lb + (ub-lb)*lhs(2, N_f)   #相当于在x为(-1，1)，t为(0，1)的区域内随机生成10000个(x，t)坐标  所以维度为(10000，2)
x_f = tf.convert_to_tensor(X_f[:,0:1], dtype=tf.float32)
t_f = tf.convert_to_tensor(np.abs(X_f[:,1:2]), dtype=tf.float32)


#generate point vectors for training
X0 = np.concatenate((x0, 0*x0), 1) # (x0, 0)    #相当于选取的100个初始条件的点
X_lb = np.concatenate((0*tb + lb[0], tb), 1) # (1, tb)  相当于在顶部边界条件上选取100个点  
X_ub = np.concatenate((0*tb + ub[0], tb), 1) # (-1, tb)  相当于在底部边界条件上选取25个点
...
fit(x_f, t_f, x0, t0, u0, x_lb, t_lb, x_ub, t_ub, col_weights, u_weights, epochs = 1000, newton_iter = 1000)
...
############################# Plotting ###############################
######################################################################
current_time = datetime.datetime.now().strftime("%H%m%d-%H%M%S")
fig_save_path = './result/' + current_time
...
ax.set_xlabel('$x$')
ax.set_ylabel('$u(t,x)$')
ax.set_title('$t = %.2f$' % float(t[25]), fontsize = 10)
ax.axis('square')
ax.set_xlim([-1.1,1.1])
ax.set_ylim([-1.1,1.1])
...
ax.set_xlim([-1.1,1.1])
ax.set_ylim([-1.1,1.1])
ax.set_title('$t = %.2f$' % float(t[50]), fontsize = 10)
...
ax.set_xlim([-1.1,1.1])
ax.set_ylim([-1.1,1.1])
ax.set_title('$t = %.2f$' % float(t[75]), fontsize = 10)
...
# plt.show()
plt.savefig(os.path.join(fig_save_path,'my_figure1.png'))
...
# plt.show()
plt.savefig(os.path.join(fig_save_path,'my_figure2.png'))

# collocation point weights
plt.scatter(t_f, x_f, c = col_weights.numpy(), s = col_weights.numpy()/5)
# plt.show()
plt.savefig(os.path.join(fig_save_path,'my_figure3.png'))
