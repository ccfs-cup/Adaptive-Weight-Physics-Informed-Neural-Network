...
checkpoint_dir = "/home/cc/CCFs/Wangf/UNPINN/checkpoints/S1/"
current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")  # tensorboard准备
log_dir = '/home/cc/CCFs/Wangf/UNPINN/logs/S1/' + current_time  # 日志文件地址
...
# save_dir = "/home/cc/CCFs/Wangf/UNPINN/case9result_pic/test[PINNpic1"
...
def LossPde(re_pde_xyt, k_rad_xy, dk_dx, dk_dy, up_nflow, dw_nflow, pde_weights):
...
@tf.function
def LossBdLogits(train_h, train_xyt, lbc_xyt, rbc_xyt, lic_xyt, ric_xyt, mic_xyt, ic_weights1,ic_weights2):
...
def train_one_step(step, epoch, train_h, train_xyt, lbc_xyt, rbc_xyt, lic_xyt, ric_xyt,mic_xyt, pde_weights, ic_weights1, ic_weights2):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss_ek, loss_bc, loss_ic, loss_mse = LossBdLogits(train_h, train_xyt, lbc_xyt, rbc_xyt, lic_xyt, ric_xyt,mic_xyt, ic_weights1, ic_weights2)
        loss_pde, loss_up_dw = LossPde(
            re_pde_xyt, k_rad_xy, dk_dx, dk_dy, up_nflow, dw_nflow, pde_weights)
        # 总损失
...
N_pde = 50000
x_pde = np.random.randint(2, 50, (N_pde, 1))
y_pde = np.random.randint(2, 50, (N_pde, 1))
t_pde = np.random.randint(2, 50, (N_pde, 1)) 
...
#权重设置
pde_weights = tf.Variable(tf.reshape(tf.repeat(100000.0, N_pde),(N_pde, -1)))
ic_weights1 = tf.Variable( 10 * tf.random.uniform([N_ric, 1]))
ic_weights2 = tf.Variable( 10 * tf.random.uniform([N_ric, 1]))
    ...
    for epoch in range(1,epochs+1):  # epoch数量
        ...
        for step, (train_xyt, train_h) in enumerate(db):
            ...
            total_loss, loss_mse, loss_pde, loss_bc, loss_ek, loss_ic, loss_up_dw = train_one_step(
                step, epoch, train_h, train_xyt, lbc_xyt, rbc_xyt, lic_xyt, ric_xyt, mic_xyt, pde_weights, ic_weights1, ic_weights2)
