...
from makedata.getdata_10 import getdata10
...
checkpoint_dir = "/home/cc/CCFs/Wangf/UNPINN/checkpoints/case10_checkpoints/"
...
lambda5 = 10  # ic
lambda6 = 100000
save_dir = "/home/cc/CCFs/Wangf/UNPINN/modelpic/PINN"
...
def plot_compared_heatmaps_3pic6(lc_xy, val_h, modeloutput, save_dir):
    step = 36
    ...
    # 保存热力图
    plt.savefig(f'{save_dir}/val_heatmap_step{step}.png')
    ...
    # 保存热力图
    plt.savefig(f'{save_dir}/modeloutput_heatmap_step{step}.png')
    plt.close()  # 关闭当前绘图，释放内存
    ...
    # 保存热力图
    plt.savefig(f'{save_dir}/difference_heatmap_step{step}.png')
    plt.close()  # 关闭当前绘图，释放内存
...
def LossPde(re_pde_xyt, k_rad_xy, dk_dx, dk_dy, up_nflow, dw_nflow, lbc_xyt1, lbc_xyt2, rbc_xyt):
...
@tf.function
def LossBdLogits(train_h, train_xyt, lic_xyt, ric_xyt,mic_xyt):
    ...
    # 过程控制损失
    loss_ek = tf.reduce_mean(tf.square(tf.abs(tf.nn.relu(logits_data - (1))))) + \
        tf.reduce_mean(tf.square(tf.abs(tf.nn.relu(0 - logits_data))))
  
    # 初始条件损失
    loss_ic = tf.reduce_mean(tf.square(lic_out - (4/10))) + \
        tf.reduce_mean(tf.square(ric_out - (6/10)))+\
...
def train_one_step(step, epoch, train_h, train_xyt, lic_xyt, ric_xyt,mic_xyt, lbc_xyt1, lbc_xyt2, rbc_xyt):
    ...
    with tf.GradientTape() as tape:
        ...
        loss_ek,  loss_ic, loss_mse = LossBdLogits(train_h, train_xyt, lic_xyt, ric_xyt,mic_xyt)
        loss_pde, loss_up_dw, loss_bc = LossPde(re_pde_xyt, k_rad_xy, dk_dx, dk_dy, up_nflow, dw_nflow, lbc_xyt1, lbc_xyt2, rbc_xyt)
        # 总损失
        total_loss = lambda1 * loss_mse + lambda2 * loss_pde + lambda3 * loss_bc + lambda4 * loss_ek + lambda5 * loss_ic + lambda6 * loss_up_dw
...
# 训练数据
(train_xyt, train_h), (test_xyt, test_h) = getdata10.get_data()  # 导入数据
...
N_mic=20000
mic_xmin, mic_xmax = 1, 51  # x 的范围 [1, 51]
    ...
    for epoch in range(1,epochs+1):  # epoch数量
        ...
        for step, (train_xyt, train_h) in enumerate(db):
            ...
            total_loss, loss_mse, loss_pde,loss_bc, loss_ek, loss_ic, loss_up_dw = train_one_step(
                step, epoch, train_h, train_xyt,lic_xyt, ric_xyt, mic_xyt, lbc_xyt1, lbc_xyt2, rbc_xyt)
        ...
        # plot_compared_heatmaps_3pic6(lc_xy, test_h*10, logits*10, save_dir)
